...
class GameWindow(QMainWindow, Ui_GameWindow):
    ...
    def random_choice(self):
        ...
        coords = [[140, 180], [260, 300], [380, 420], [500, 540], [620, 660]]
        self.coords_y = []
        for i in range(1, 6):
    ...
    def check(self):
        self.result = []
        self.errors = []
        self.resolution_ = [1 for i in range(5) if 400 <= self.ans_buttons[i].x() <= 440 and 140 <= self.ans_buttons[i].y() <= 680]

        if (400 <= self.ans_buttons[0].x() <= 440) and (self.coords_y[0][0] <= self.ans_buttons[0].y() <= self.coords_y[0][1]):
            self.result.append(1)
        else:
            self.errors.append([self.factors_1[0], self.factors_2[0], self.ans[0]])
            
        if (400 <= self.ans_buttons[1].x() <= 440) and (self.coords_y[1][0] <= self.ans_buttons[1].y() <= self.coords_y[1][1]):
            self.result.append(2) 
        else:
            self.errors.append([self.factors_1[1], self.factors_2[1], self.ans[1]])
        
        if (400 <= self.ans_buttons[2].x() <= 440) and (self.coords_y[2][0] <= self.ans_buttons[2].y() <= self.coords_y[2][1]):
            self.result.append(3)
        else:
            self.errors.append([self.factors_1[2], self.factors_2[2], self.ans[2]])
        
        if (400 <= self.ans_buttons[3].x() <= 440) and (self.coords_y[3][0] <= self.ans_buttons[3].y() <= self.coords_y[3][1]):
            self.result.append(4)
        else:
            self.errors.append([self.factors_1[3], self.factors_2[3], self.ans[3]])
        
        if (400 <= self.ans_buttons[4].x() <= 440) and (self.coords_y[4][0] <= self.ans_buttons[4].y() <= self.coords_y[4][1]):
...
class InvisibleWindow(QMainWindow):
    ...
    def button_clicked_groupgame1(self):   
        ...
        #if self.group_gamewindow_RUN.check() == 5:
